...
# Modified by Chelsea to work with Ultralytics and PyTorch!
...
def process_thread(args, request_queue, response_queue):
    ...
    for model in args.model:
        this_model = UltralyticsObjectDetectionModel(model)
...
def main(argv):
    ...
    parser.add_argument(
        '-m', '--model', help=
        '[MODEL_FILE]: Path to a YOLO model\'s .pt file',
    ...
    for model in options.model:
        if not os.path.isfile(model):
            print('Error: model file (' + model + ') not found or is not a file.')
            sys.exit(1)
