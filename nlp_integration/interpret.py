...
class Phase1Interpreter:
    ...
    def classify_intent(self, text: str) -> IntentResult:
        ...
        elif best_score < 0.60 and (best_score - second_score) < self.delta_threshold:
            is_ambiguous = True
            reason = f"Top intents too close (Δ = {best_score - second_score: .2f})."

        return IntentResult(best_label, best_score, is_ambiguous, reason)
    ...
    def interpret(self, text: str) -> Dict:
        ...
        questions = self.clarification_questions(intent, objects)
